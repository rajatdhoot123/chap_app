import React from "react";
import { connect } from "react-redux";
import { Link } from "react-router-dom";
import Header from "./Header.js";
import css1 from "tachyons";
import css from "../css/app.css";


class Home extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
      id: 3,
      display: "none",
      cName: "noBox col-md-12 col-sm-12",
      amaName: "col-sm-6 col-md-6"
    };
    this.toggleId = this.toggleId.bind(this);
    this.toggleDisplay = this.toggleDisplay.bind(this);
  }

  toggleDisplay(event) {
    if (this.state.display == "block") {
      this.setState({
        cName: "noBox col-md-12 col-sm-12",
        display: "none"
      });
    }
  }

  toggleId(event) {
    var id = parseInt(event.target.id);
    if (this.state.display == "none") {
      this.setState({
        id: event.target.id,
        cName: "showBox col-md-12 col-sm-12",
        display: "block"
      });
    } else {
      this.setState({
        id: event.target.id,
        cName: "noBox col-md-12 col-sm-12",
        display: "none"
      });
    }
  }

  componentDidMount() {
    this.setState({
      loading: false
    });
  }

  render() {
    if (this.state.loading) {
      return <h1>loading......</h1>;
    } else {
      console.log("this.props.ama.2.name", this.props.ama[2].name);
      console.log("after state: ", this.state.id);
      return (
        <div>
          <div className="nav">
            <Link to="/host">Host View</Link>
            <Link to="/admin">Admin View</Link>
            <Link to="/founder">Founder View</Link>
          </div>
          <Header />

          <div className="container">
            <div className="row">
              <div className="col-sm-6 col-md-6">
                <p className="notbold1"> LetsVenture Ask Me Anything </p>
                <p>
                  One liner for the LetsVenture Ask me Anything feature.
                  Basically its setting the context about what is this all about
                </p>
              </div>
            </div>

            <div className="row">
              <div className="col-sm-12 col-md-12">
                <div className="center">
                  <img
                    className="cover"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src="data:image/svg+xml;base64,"
                    data-holder-rendered="true"
                  />
                </div>
              </div>
            </div>
            <div className="row">
              <div className="col-sm-5 col-md-5">
                <div className="center">
                  <img
                    className="thumbnail"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src="data:image/svg+xml;base64,"
                    data-holder-rendered="true"
                  />
                </div>
              </div>
              <div className="col-sm-7 col-md-7">
                <div className="center">
                  <img
                    className="thumbnail"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src="data:image/svg+xml;base64,"
                    data-holder-rendxered="true"
                  />
                </div>
              </div>
              <div className="body-header">
                <p className="notbold1 new"> Upcoming AMA </p>
              </div>

              <div
                className={this.state.cName}
                id="box"
              >
                <div className="col-md-2 col-sm-2" onClick={this.toggleDisplay}>
                  <img
                    className="circle img-circle"
                    src={this.props.ama[this.state.id].url}
                    alt="Cinque Terre"
                  />
                </div>
                <div className="col-md-8 col-sm-8" onClick={this.toggleDisplay}>
                  <h3>
                    {" "}AMA with {this.props.ama[this.state.id].name}
                  </h3>
                  <p>
                    {this.props.ama[this.state.id].day},{" "}
                    {this.props.ama[this.state.id].time}
                  </p>
                  <p>Moderated By: LetsVenture Team</p>
                </div>
                <div className="col-md-1 col-sm-1 right">
                  <button className="btn btn-default" role="button">
                    Remind Me
                  </button>
                </div>
                <div className="col-md-12 col-sm-12" onClick={this.toggleDisplay}>
                  <hr />
                  <div className="col-md-8 col-sm-8 ">
                    <p>
                      {this.props.ama[this.state.id].description}
                    </p>
                    <p>You can ask questions around:</p>
                    <p>1. Making things happen with Music</p>
                    <p>2. How to make investments and see them through</p>
                    <a href="https://letsventure.com/">
                      View {this.props.ama[this.state.id].name}'s Portfolio on
                      LetsVenture
                    </a>
                  </div>
                </div>
              </div>
              <div className={this.state.amaName}>
                <div className=" center">
                  <img
                    className="thumbnail"
                    id="2"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[2].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>

              <div className={this.state.amaName}>
                <div className="center">
                  <img
                    className="thumbnail"
                    id="3"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[3].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>
              <div className={this.state.amaName}>
                <div className="center">
                  <img
                    className="thumbnail"
                    id="4"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[4].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>
              <div className={this.state.amaName}>
                <div className="center">
                  <img
                    className="thumbnail"
                    id="5"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[5].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>
              <div className={this.state.amaName}>
                <div className="center">
                  <img
                    className="thumbnail"
                    id="6"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[6].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>
              <div className={this.state.amaName}>
                <div className="center">
                  <img
                    className="thumbnail"
                    id="7"
                    alt="100%x180"
                    data-src="holder.js/100%x180"
                    src={this.props.ama[7].url}
                    data-holder-rendered="true"
                    onClick={this.toggleId}
                  />
                </div>
              </div>
              <div className="button center">
                <button className="btn btn-default" role="button">
                  Show More
                </button>
              </div>
            </div>
          </div>
        </div>
      );
    }
  }
}

const mapStateToProps = state => {
  return state;
};

export default connect(mapStateToProps)(Home);
